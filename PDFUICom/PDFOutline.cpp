...
namespace winrt::RDUILib::implementation
{
    ...
    {
        PDF_OUTLINE hand = Document_getOutlineNext(m_doc, m_hand);
    ...
    {
        PDF_OUTLINE hand = Document_getOutlineChild(m_doc, m_hand);
        if (!hand) return nullptr;
        return winrt::make<implementation::PDFOutline>((int64_t)m_doc, (int64_t)hand);
    }
    bool PDFOutline::AddNext(winrt::hstring label, int32_t dest, float y)
    {
        return Document_addOutlineNext(m_doc, m_hand, label.c_str(), dest, y);
    }
    bool PDFOutline::AddChild(winrt::hstring label, int32_t dest, float y)
    {
        return Document_addOutlineChild(m_doc, m_hand, label.c_str(), dest, y);
    }
    bool PDFOutline::RemoveFromDoc()
    {
        bool ret = Document_removeOutline(m_doc, m_hand);
        m_doc = NULL;
        m_hand = NULL;
        return ret;
    }
    ...
    {
        return Document_getOutlineLabel(m_doc, m_hand);
    }
    int32_t PDFOutline::dest()
    {
        return Document_getOutlineDest(m_doc, m_hand);
    }
}
