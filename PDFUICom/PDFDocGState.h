...
namespace winrt::RDUILib::implementation
{
    struct PDFDocGState : PDFDocGStateT<PDFDocGState>
    {
        ...
        {
            Document_setGStateFillAlpha(m_doc, m_hand, alpha);
        }
        void SetStrokeAlpha(int32_t alpha)
        {
            Document_setGStateStrokeAlpha(m_doc, m_hand, alpha);
        }
        void SetStrokeDash(winrt::array_view<float const> dash, float phase)
        {
            Document_setGStateStrokeDash(m_doc, m_hand, dash.data(), dash.size(), phase);
        }
        void SetBlendMode(int32_t bmode)
        {
            Document_setGStateBlendMode(m_doc, m_hand, bmode);
        }
        PDF_DOC m_doc;
