...
namespace winrt::RDUILib::implementation
{
    int32_t PDFDoc::CreatePath(winrt::hstring path)
    {
        ...
        PDF_ERR err;
        m_doc = Document_createForPath(stxt, &err);
        return err;
    }
    ...
    int32_t PDFDoc::OpenPath(winrt::hstring path, winrt::hstring password)
    {
        ...
        PDF_ERR err;
        m_doc = Document_openPathW(path.c_str(), stxt, &err);
        return err;
    }
    int32_t PDFDoc::Open(IRandomAccessStream stream, winrt::hstring password)
    {
        ...
        WideCharToMultiByte(CP_ACP, 0, password.c_str(), -1, stxt, 63, NULL, NULL);
        PDF_ERR err;
        m_doc = Document_open(stream, stxt, &err);
        return err;
    }
    ...
    {
        int64_t hand = (int64_t)Document_getPage(m_doc, pageno);
    ...
    {
        int64_t hand = (int64_t)Document_getOutlineNext(m_doc, NULL);
        if (!hand) return nullptr;
    ...
    {
        return Document_addOutlineNext(m_doc, NULL, label.c_str(), dest, y);
    }
    int32_t PDFDoc::Permission()
    {
        return Document_getPermission(m_doc);
    }
    ...
    {
        return Document_verifySign(m_doc, (PDF_SIGN)sign.Handle());
    }
    winrt::RDUILib::PDFPage PDFDoc::NewPage(int32_t pageno, float w, float h)
    {
        int64_t hand = (int64_t)Document_newPage(m_doc, pageno, w, h);
        if (!hand) return nullptr;
        return winrt::make<implementation::PDFPage>(hand);
    }
    ...
    {
        int64_t hand = (int64_t)Document_importStart(m_doc, (PDF_DOC)src.Handle());
        return (!hand) ? nullptr : winrt::make<implementation::PDFImportor>((int64_t)m_doc, hand);
    }
    winrt::RDUILib::PDFDocImage PDFDoc::NewImage0(winrt::Microsoft::UI::Xaml::Media::Imaging::WriteableBitmap bitmap, bool has_alpha)
    {
        int64_t hand = (int64_t)Document_newImage(m_doc, bitmap.try_as<winrt::Windows::UI::Xaml::Media::Imaging::WriteableBitmap>(), has_alpha);
        return (!hand) ? nullptr : winrt::make<implementation::PDFDocImage>(hand);
    }
    winrt::RDUILib::PDFDocImage PDFDoc::NewImage1(winrt::Windows::Graphics::Imaging::SoftwareBitmap bitmap, bool has_alpha)
    {
        int64_t hand = (int64_t)Document_newImage2(m_doc, bitmap, has_alpha);
        return (!hand) ? nullptr : winrt::make<implementation::PDFDocImage>(hand);
    }
    winrt::RDUILib::PDFDocImage PDFDoc::NewImage2(winrt::Microsoft::UI::Xaml::Media::Imaging::WriteableBitmap bitmap, int32_t matte)
    {
        int64_t hand = (int64_t)Document_newImageMatte(m_doc, bitmap.try_as<winrt::Windows::UI::Xaml::Media::Imaging::WriteableBitmap>(), matte);
        return (!hand) ? nullptr : winrt::make<implementation::PDFDocImage>(hand);
    }
    winrt::RDUILib::PDFDocImage PDFDoc::NewImage3(winrt::Windows::Graphics::Imaging::SoftwareBitmap bitmap, int32_t matte)
    {
        int64_t hand = (int64_t)Document_newImage2Matte(m_doc, bitmap, matte);
        return (!hand) ? nullptr : winrt::make<implementation::PDFDocImage>(hand);
    }
    winrt::RDUILib::PDFDocImage PDFDoc::NewImageJPEG(winrt::hstring path)
    {
        char stxt[512];
        WideCharToMultiByte(CP_ACP, 0, path.c_str(), -1, stxt, 511, NULL, NULL);
        int64_t hand = (int64_t)Document_newImageJPEG(m_doc, stxt);
        return (!hand) ? nullptr : winrt::make<implementation::PDFDocImage>(hand);
    }
    winrt::RDUILib::PDFDocImage PDFDoc::NewImageJPX(winrt::hstring path)
    {
        char stxt[512];
        WideCharToMultiByte(CP_ACP, 0, path.c_str(), -1, stxt, 511, NULL, NULL);
        int64_t hand = (int64_t)Document_newImageJPX(m_doc, stxt);
        return (!hand) ? nullptr : winrt::make<implementation::PDFDocImage>(hand);
    }
